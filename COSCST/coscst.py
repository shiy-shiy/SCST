...
def myloss(true_label,pred_label):
    #crossentropy1 = tf.keras.losses.categorical_crossentropy(true_label[0], pred_label[0])
    #crossentropy2 = tf.keras.losses.categorical_crossentropy(true_label[1], pred_label[1])
    crossentropy1 = tf.keras.losses.MeanSquaredError(true_label[0], pred_label[0])
    crossentropy2 = tf.keras.losses.MeanSquaredError(true_label[1], pred_label[1])
    crossentropy = crossentropy1+crossentropy2  #
    #crossentropy = crossentropy1+crossentropy2  #
...
#set seed
    ...
    ################
    '''
    y_pred=pd.Series(np.argmax(q_pred,axis=1),index=adata.obs.index,dtype='category')#todo
    mylabel=y_pred.cat.categories
    print("mylabel",mylabel)
    adata.obs['mylabel']=y_pred #输出未排序的spot
    print("y_pred",y_pred)
    ...
    tsnefeatures=np.load(os.path.join(save_dir,"tsnefeatures.npy"),allow_pickle=True)
    print('tsnefeatures.shape',tsnefeatures.shape)
    adata.obsm['X_Embeded_original']=np.array(tsnefeatures)#todo
    #features=self.extract_features(self.x)
    #adatas=sc.AnnData(features)
    #tsnefeatures=pd.DataFrame(adatas.X)#save autoencoder feature to do mytsne
    
    tsnefeatures=np.load(os.path.join(save_dir,"tsnefeatures.npy"),allow_pickle=True)
    print('tsnefeatures.shape',tsnefeatures.shape)
    adata.obsm['X_Embeded_original']=np.array(tsnefeatures)
    #Embedded_z=self.extract_features(self.x)
    #adata.obsm["X_Embeded_z"]=Embedded_z[1]
    
    if do_mytsne:
        num_Cores_tsne=int(num_Cores_tsne) if total_cpu>int(num_Cores_tsne) else int(math.ceil(total_cpu/2))
        sc.tl.tsne(adata,use_rep='X_Embeded_original',learning_rate=learning_rate,perplexity=perplexity,n_jobs=num_Cores_tsne)
        adata.obsm['X_mytsne']=adata.obsm["X_tsne"].copy()
        print('added X_mytsne into the umap coordinates (adata.obsm)\n')
    ...
    if do_myumap:
        #X_Embeded_original 
        sc.pp.neighbors(adata,n_neighbors=n_neighbors,use_rep="X_Embeded_original") 
        sc.tl.umap(adata)
        adata.obsm["X_myumap"]=adata.obsm["X_umap"].copy() 
        print('umap finished and added X_myumap into the umap coordinates (adata.obsm)\n')
        #sc.logging.msg(' umap finished', t=True, end=' ', v=4)
        #sc.logging.msg('and added\n'        
        #        '    \'X_umap\''+str(louvain_resolution),'the umap coordinates (adata.obsm)\n', v=4)
        #del adata.uns["neighbors"]

    if do_myumap_X_Embeded_z:
        #X_Embeded_original 
        sc.pp.neighbors(adata,n_neighbors=n_neighbors,use_rep="X_Embeded_z") 
        sc.tl.umap(adata)
        adata.obsm["X_myumap_X_Embeded_z"]=adata.obsm["X_umap"].copy() 
        print('umap finished and added X_myumap into the umap coordinates (adata.obsm)\n')
        #sc.logging.msg(' umap finished', t=True, end=' ', v=4)
        #sc.logging.msg('and added\n'        
        #        '    \'X_umap\''+str(louvain_resolution),'the umap coordinates (adata.obsm)\n', v=4)
...
def train(data,dims=None,
    ...
    for ith,resolution in enumerate(louvain_resolution):
	    ...
	    do_mytsne=do_mytsne,
        do_myumap=do_myumap,
        do_myumap_X_Embeded_z=do_myumap_X_Embeded_z,
	    do_tsne=do_tsne,
...
if __name__=='__main__':
    ...
    adata=sc.read("/s/f/shiyi/ST/Data/test/0405combineddata_norm_scale.h5ad")
    '''
